...
| F5 | `database.py`, `handlers/meds.py`, `scheduler.py` | **Напоминание о заканчивающихся таблетках** — указать количество в упаковке, бот предупредит, когда осталось на N дней. **Модель данных**: текущая `dosage` — свободный текст («500мг», «1 таблетка»), парсить нельзя → нужны явные числовые поля. Добавить в `medications`: `stock_qty REAL` (текущий остаток, NULL = трекинг выключен), `units_per_dose REAL DEFAULT 1` (единиц за приём), `low_stock_days INTEGER DEFAULT 5` (порог предупреждения в днях). **Декремент**: в `handle_intake_callback` при `taken` уменьшать `stock_qty` на `units_per_dose` (откат при смене taken→skipped — edge-case, решить). **Действие «Пополнить»** — выставить/прибавить остаток. **«Осталось на N дней»** для любых частот: проецировать вперёд по дням через `_rule_fires_today`, суммируя расход до исчерпания (общий хелпер «положенных приёмов» из F2/F3). **Предупреждение**: ежедневная проверка в планировщике (рядом с планом дня) — «⚠️ <Лекарство> скоро закончится: осталось на N дн. (M шт.)». **Связь с F4**: на паузе не списывать и не предупреждать. **Концептуально**: `medications` = контейнер (название, дозировка, **остаток**), `schedule_rules` = план приёма (уже создаётся/редактируется/удаляется; пауза — F4) — модель НЕ требует большого рефакторинга, только новые колонки. Caregiver-aware |
